...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    Ticket::new(title.clone(), description.clone(), status.clone()).unwrap_or_else(|err| {
        match err {
            TicketNewError::TitleErr(e) => { panic!("{}", e.as_str()) }
            TicketNewError::DesErr(_) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleErr("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleErr("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DesErr("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DesErr("Description cannot be longer than 500 bytes".to_string()));
        }
